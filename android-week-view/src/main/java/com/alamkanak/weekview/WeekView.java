...
/**
 * 关于该控件在XML中的元素属性的说明：
 * 1.headerColumnPadding：顶部的头组件的填充的宽度；
 */
public class WeekView extends View {
    ...
    private Paint mSubHourSeparatorPaint;
    ...
    private int mHourSeparatorColor = Color.LTGRAY;
    private int mSubHourSeparatorColor = Color.rgb(230, 230, 230);
...
//        mHeaderTextPaint.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        // “小时”内部中间的分界线的笔刷
        mSubHourSeparatorPaint = new Paint();
        mSubHourSeparatorPaint.setStyle(Paint.Style.STROKE);
        mSubHourSeparatorPaint.setStrokeWidth(mHourSeparatorHeight);
        mSubHourSeparatorPaint.setColor(mSubHourSeparatorColor);
...
//        mTodayHeaderTextPaint.setTypeface(Typeface.DEFAULT_BOLD); // 将其设置为非粗体的
        ...
        mPrimaryColorPaint.setTextAlign(Paint.Align.CENTER);
        mPrimaryColorPaint.setTextSize(mTextSize); // 文字的大小在需要的时候自己再进行设置
        mPrimaryColorPaint.setColor(primaryColor);

        // 强调色的绘制刷
        mAccentColorPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mAccentColorPaint.setTextAlign(Paint.Align.CENTER);
        mAccentColorPaint.setTextSize(mTextSize); // 文字的大小在需要的时候自己再进行设置
        mAccentColorPaint.setColor(accentColor);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        mWidthPerDay = mWidthPerDay / mNumberOfVisibleDays; // 每天的宽度

        calculateHeaderHeight(); // 确保“头”的尺寸正确（取决于“全天事件”）
        ...
        float[] subHourLines = new float[lineCount * 4];
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                ...
                if (top > mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0){
                    ...
                    subHourLines[i * 4] = start;
                    subHourLines[i * 4 + 1] = top + mHourHeight * 0.5f;
                    subHourLines[i * 4 + 2] = startPixel + mWidthPerDay;
                    subHourLines[i * 4 + 3] = top + mHourHeight * 0.5f;
                    i++;
                }
            }
            ...
            // 绘制“小时”中间的分界线
            canvas.drawLines(subHourLines, mSubHourSeparatorPaint);
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            if (sameDay) { // 今天
                canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, mTodayHeaderTextPaint);
            } else if (day.get(Calendar.DAY_OF_MONTH) == 1) { // 月首
                canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, mPrimaryColorPaint);
            } else { // 其他情况
     ...
     * 检查滚动是否应该被停止
     *
     * @return 如果滚动在动画停止之前应该被停止就返回true
     */
    private boolean forceFinishScroll() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH && mScroller.getCurrVelocity() <= mMinimumFlingVelocity;
    }

    // region 公开的方法 setter 和 getter

    /**
     * 设置当前是否为暗色的主题
     *
     * @param isDarkTheme true 表示暗色主题
     */
    public void setIsDarkTheme(boolean isDarkTheme){
        this.isDarkTheme = isDarkTheme;
        if (isDarkTheme){
            mHeaderBackgroundPaint.setColor(getResources().getColor(R.color.dark_theme_foreground));
            mHeaderTextPaint.setColor(Color.WHITE);
            mTimeTextPaint.setColor(Color.WHITE);
            mDayBackgroundPaint.setColor(getResources().getColor(R.color.dark_theme_foreground));
            mPastBackgroundPaint.setColor(getResources().getColor(R.color.dark_theme_background));
            mFutureBackgroundPaint.setColor(getResources().getColor(R.color.dark_theme_foreground));
            mHourSeparatorPaint.setColor(getResources().getColor(R.color.white_divider_color));
            mSubHourSeparatorPaint.setColor(getResources().getColor(R.color.second_white_divider_color));
        }
        invalidate();
    }
    ...
    /**
     * 垂直滚动到指定的时间，比如0.5就表示0点30分钟
     *
     * @param hour 要滚动到的时间是24小时制的，支持的数值在0到24之间
     */
    ...
    /**
     * 获取DateTimeInterpreter，它提供文字给“头”组件来进行绘制
     *
     * @return 时间和日期解释器，本质上就是对时间和日期进行格式化之后返回
     */
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        // 当用于绘制的列比较宽，可以显示较长的日期
                        if (mDayNameLength == LENGTH_SHORT) {
                            return new SimpleDateFormat("EEEEE M/dd", Locale.getDefault()).format(date.getTime()).toUpperCase();
                        }

                        // 当用于绘制的列比较窄，显示较短的日期
                        if (date.get(Calendar.DAY_OF_MONTH) == 1) { // 每个月的第一天
                            return new SimpleDateFormat("EEE MMM", Locale.getDefault()).format(date.getTime()).toUpperCase();
                        }

                        return new SimpleDateFormat("EEE-dd", Locale.getDefault()).format(date.getTime()).toUpperCase();
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                    }
                ...
                public String interpretTime(int hour) {
                    ...
                    try {
//                        SimpleDateFormat sdf = DateFormat.is24HourFormat(getContext()) ? new SimpleDateFormat(" HH:00 ", Locale.getDefault()) : new SimpleDateFormat(" hh:00 ", Locale.getDefault());
    ...
    /**
     * 设置当前日历上可见的天数，取值1,3,7
     *
     * @param numberOfVisibleDays 设置当前日历上可见的天数
     */
    ...
    public int getSubHourSeparatorColor() {
        return mSubHourSeparatorColor;
    }

    /**
     * 设置“小时”中间的分界线的笔刷颜色
     *
     * @param mSubHourSeparatorColor “小时”中间的分界线的笔刷颜色
     */
    public void setSubHourSeparatorColor(int mSubHourSeparatorColor) {
        this.mSubHourSeparatorColor = mSubHourSeparatorColor;
        mSubHourSeparatorPaint.setStrokeWidth(mSubHourSeparatorColor);
        invalidate();
    }
     ...
     * “今天”的顶部的“头”组件的颜色
     *
     * @param todayHeaderTextColor 今天的“头”组件的颜色
     */
